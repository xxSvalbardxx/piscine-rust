pub fn talking(text: &str) -> &str {
    match text {
        ...
        _ if text.pop().chars().all(|c| c.is_uppercase()) && text.ends_with("?") && text.len() > 0 => "Quiet, I am thinking!",
        _ if text.pop().chars().all(|c| c.is_uppercase()) && text.ends_with("!") && text.len() > 0 => "There is no need to yell, calm down!",
